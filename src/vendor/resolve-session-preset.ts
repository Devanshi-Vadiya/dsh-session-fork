/**
 * VENDORED FROM: deepseek-harness@2h8a0a8cff (b8dfa8b892~1, copied 2026-08-29)
 * — packages/preset/agent-presets/src/session.ts:16-54 (event-map
 * augmentation, `PresetBearingSession`, `resolveSessionPreset`).
 *
 * Upstream removed the `resolveSessionPreset` export in b8dfa8b892
 * (2026-08-25) in favor of the `agentPreset` session projection
 * (`agentPresetProjectionDefinition`); that projection runs in the host, but
 * fork reconstruction resolves the preset from a stored log, so the helper is
 * vendored verbatim. The event-map augmentation travels with it: nothing else
 * in this package imports @deepseek-ai/dsh-agent-presets anymore, so this
 * file is the plugin's only declaration of the `agent-preset/selected`
 * event. Verbatim copy — no [fork:adapt]/[fork:surgery] markers.
 * @module dsh-session-fork/src/vendor/resolve-session-preset
 */
